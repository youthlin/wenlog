...
func registerAuthRoutes(r *gin.Engine, auth *handler.Auth) {
	...
	r.POST("/auth/passkey/begin", loginLimiter, auth.BeginPasskeyLogin)
	r.POST("/auth/passkey/finish", loginLimiter, auth.FinishPasskeyLogin)
...
func registerAdminRoutes(r *gin.Engine, adm *handler.Admin) {
	...
	g.POST("/profile/two-factor/enable", adm.EnableTwoFactor)
	g.POST("/profile/two-factor/disable", adm.DisableTwoFactor)
	g.POST("/profile/passkeys/begin", adm.BeginPasskeyRegistration)
	g.POST("/profile/passkeys/finish", adm.FinishPasskeyRegistration)
	g.POST("/profile/passkeys/:id/delete", adm.DeletePasskey)
	...
	contentGroup.POST("/comments/reply/:id", adm.ReplyComment)
